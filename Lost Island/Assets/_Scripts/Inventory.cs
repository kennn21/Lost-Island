...
public class Inventory : MonoBehaviour {
	...
	void Update() {
		...
		if((Input.GetAxisRaw("Mouse ScrollWheel") != 0 && !player.ActiveMenu())) {
			if(placingStructure) {
				CancelStructurePlacement();
			}
		...
		if(!player.dead) {
			...
			if(currentSelectedItem && !player.ActiveMenu()) {
				if(Input.GetMouseButtonDown(0)) {
					...
					InventoryUpdate();
				} else if(Input.GetKeyDown(dropKey) && currentSelectedItem.type != Item.ItemType.Structure) {
...
/*		audioManager.Play("UIClick");*/
	...
	public void DropItem(Item item, int amount) {
		for(int i = 0; i < amount; i++) {
			GameObject itemObj = Instantiate(item.prefab, player.playerCamera.transform.position + player.playerCamera.transform.forward * 12.5f + Vector3.forward * i * (item.prefab.GetComponentInChildren<Renderer>().bounds.size.y + 0.1f), player.playerCamera.transform.rotation);
